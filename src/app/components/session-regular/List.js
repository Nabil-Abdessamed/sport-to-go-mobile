...
import SessionRegularItem from "./SessionRegularItem";
import Styles from "./SessionRegularStyles";
import SessionRegularItemGrid from "./SessionRegularItemGrid";
...
export default function SessionRegularList({
  numColumns = 1,
  data = null,
  mode = MODE.GRID,
  loadMore = false,
  ...
  return data ? (
    <FlatList
      ...
      data={data.items}
      keyExtractor={(_, index) => `session-regular-item-${index}`}
      renderItem={({ item }) =>
        mode === MODE.GRID ? (
          <SessionRegularItemGrid
            t={t}
            session={item}
